 ...
 */
 jQuery.fn.extend({Calculadora: function(op) {
					...
					Botones=op.Botones;
					Signos=op.Signos;
                    $(LaCalculadora).append('<input type="hidden" class="'+op.ClasetxtSalida+'" id="'+op.txtSalida+'" value="0" >');
					...
					$('.'+NombreBotonesClase).click(function(){
						...
						if(vTecla=='='){salida.val(eval(salida.val()));
							...
							if(Signos.indexOf(vTecla)>-1){salida.val(0)}
								...
								$("#montopagado").val(vTecla);
								$("#lblpagado").text(vTecla);
							}
						}else{salida.val(salida.val()+vTecla);
							$("#montopagado").val(salida.val());
								$("#lblpagado").text(salida.val());
						  } 
						}
					});
